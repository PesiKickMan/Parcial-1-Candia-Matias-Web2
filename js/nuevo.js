...
function renderProducto() {
    axios.get(`${baseURL}/productos`)
        ...
        .catch(error => {
            swal("Error al editar el producto", "Código: " +  error, "error");
        })
}
    ...
    axios.delete(`${baseURL}/productos/${idProducto}`)
        .then(response => {
            if (response.status === 200) {
                ...
                alert('Error al eliminar el producto. Codigo: ' + response.status);
            }
        })
        .catch(error => {
            if(error.response) {
                alert('Error al eliminar el producto. Codigo: ' + error.response.status);
            } else {
